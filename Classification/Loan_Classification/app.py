...
<style>
    ...
    .hero-box h1 { color: white; font-size: 2.2rem; font-weight: 700; margin: 0 0 0.5rem 0; }
    .hero-box p { color: rgba(255,255,255,0.85); font-size: 1rem; margin: 0; }
    ...
    .result-approved h2 { color: #155724; margin: 0 0 0.5rem 0; font-size: 1.6rem; }
    .result-rejected h2 { color: #721c24; margin: 0 0 0.5rem 0; font-size: 1.6rem; }
    .result-approved p { color: #155724; margin: 0; font-size: 0.95rem; }
    .result-rejected p { color: #721c24; margin: 0; font-size: 0.95rem; }

    .factor-good {
        background: #f0fdf4; border: 1px solid #bbf7d0; border-radius: 8px;
        padding: 0.7rem 1rem; margin: 0.4rem 0; color: #166534; font-size: 0.9rem;
    }
    .factor-bad {
        background: #fff7ed; border: 1px solid #fed7aa; border-radius: 8px;
        padding: 0.7rem 1rem; margin: 0.4rem 0; color: #9a3412; font-size: 0.9rem;
    }
    .factor-neutral {
        background: #f0f9ff; border: 1px solid #bae6fd; border-radius: 8px;
        padding: 0.7rem 1rem; margin: 0.4rem 0; color: #0c4a6e; font-size: 0.9rem;
    }
    ...
    .tip-box {
        padding: 0.7rem 1rem; background: #fffbeb; border: 1px solid #fde68a;
        border-radius: 8px; margin: 0.4rem 0; color: #92400e; font-size: 0.9rem;
    }
    
    /* Fix for Selectbox and Input visibility */
...
if predict:
    ...
    try:
        ...
        try:
            ...
            approval_pct = min(max(float(proba[1]) * 100, 0), 100)
    ...
    # ── Result ──
    # Result display section update
    if prediction == 1:
        st.markdown(f"""<div class="result-approved">
            <h2>✅ LOAN APPROVED</h2>
            <p>Model predicts <strong>{approval_pct:.1f}% approval probability</strong>.</p>
        </div>""", unsafe_allow_html=True)
    else:
        # ikkada rejection probability kakunda approval thakkuva undhani chupisthunnam
        st.markdown(f"""<div class="result-rejected">
            <h2>❌ LOAN REJECTED</h2>
            <p>Model predicts only <strong>{approval_pct:.1f}% approval probability</strong>.</p>
        </div>""", unsafe_allow_html=True)
