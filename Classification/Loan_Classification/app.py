...
if st.button("🔍 Check Loan Eligibility", use_container_width=True, type="primary"):
    ...
    # ── Prediction Block ──
    try:
        # Step 1: Feature order match
        if hasattr(model, 'feature_names_in_'):
            input_df = input_df[model.feature_names_in_]
        
        # Step 2: Use the underlying tree directly to skip attribute checks
        # Ee line chala important
        prediction = model.predict(input_df.values)[0]
        proba = model.predict_proba(input_df.values)[0]
        confidence = max(proba) * 100
        
    except Exception as e:
        # Inka error vasthe, manam manual ga attributes ni clean cheyali
