...
@st.cache_resource
def load_model():
    # 1. Get the path to the folder where app.py is located
    current_dir = os.path.dirname(__file__)
    
    # 2. Join that path with your model filename
...
if st.button("🔍 Check Loan Eligibility", use_container_width=True, type="primary"):
    ...
    # ── Prediction Block ──
    try:
        # Step 1: Feature order match cheyandi (indhuvalla column mismatch raadhu)
        if hasattr(model, 'feature_names_in_'):
            input_df = input_df[model.feature_names_in_]
        
        # Step 2: .values vaadi predict cheyandi
        # Ila chesthe scikit-learn internal checks ni skip chesi direct ga result isthundhi
        ...
        # Edhaina error vasthe ikada kanipisthundhi
